...
const (
	...
	BUCKET_NAME = "around-bucket-jiang"

	API_PREFIX = "/api/v1"
)

// "image""video" are treated differently by the frontend
var (
...
func main() {
	...
	// use a router to map url to handlerfunctions and set the method
	...
	r := mux.NewRouter()
	// OPTIONS is a http method for inter-domain to verify whether other methods are allowed
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	// generate a new id for the posted file
	id := uuid.New()
	...
	// add face score to the image
	// Sprintf: format print to a string
	if p.Type == "image" {
...
func saveToGCS(r io.Reader, objectName string) (*storage.ObjectAttrs, error) {
	...
	// access control, all users are allowed to read
...
// a handler to search a cluster of files with similar properties
// here is face
// http.ResponseWriter is an interface; it doesn't have a pointer
